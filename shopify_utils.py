...
def update_product_fields(product_gid: str, api_key: str, store_url: str, title: str = None, body_html: str = None, handle: str = None, status: str = None) -> dict:
    """
    Updates specified Shopify product fields via GraphQL mutation.
    Returns the API response dict from helper.
    """
    if not product_gid or not store_url or not api_key:
        logger.error("❌ update_product_fields: Missing required args (GID, key, URL).")
        return {"_success": False, "_error": True, "_message": "Missing required arguments."}
    ...
    if len(input_payload) <= 1:
        logger.warning("⚠️ update_product_fields called but no fields specified.")
        return {"_success": True, "_message": "No fields to update.", "_status_code": None}

    mutation = """
    mutation productUpdate($input: ProductInput!) {
      productUpdate(input: $input) {
        product { id title handle status bodyHtml }
    ...
    fields_to_update = list(input_payload.keys() - {'id'})
    logger.info(f"🔄 Updating product fields for GID: {product_gid}. Fields: {fields_to_update}")
    # Assumes shopify_graphql_request is defined/imported in this file and returns dict
    response = shopify_graphql_request(store_url, api_key, mutation, variables)

    if response and response.get("_success") and not response.get("_user_errors_exist"):
        logger.info(f"✅ Product update successful for GID: {product_gid}")
        return response
    elif response and response.get("_user_errors_exist"):
        logger.error(f"❌ Shopify userErrors on update GID {product_gid}: {response.get('data', {}).get('productUpdate', {}).get('userErrors')}")
        if isinstance(response, dict): response["_success"] = False # Ensure flag reflects error
        return response
    else:
        error_msg = response.get("_message", "Unknown GraphQL error") if response else "No response"
        logger.error(f"❌ GraphQL update failed for {product_gid}. Message: {error_msg}")
        return response if response else {"_success": False, "_error": True, "_message": "Update request failed"}
